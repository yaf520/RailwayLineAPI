...
public:
    ///连续里程+投影距离 -> 坐标+切线角
    virtual bool TrsCmlDistToNE(double dCml,  double dDist, double& dX, double& dY, double& dAngle) = 0;
    
    ///坐标 -> 连续里程+投影距离+切线角集合
    virtual uint32_t TrsNEToCmlDist(double dX, double dY, double arrCml[s_nMaxArrCount], double arrDist[s_nMaxArrCount], double arrAngle[s_nMaxArrCount]) = 0;
    
    ///里程->高程+俯仰角
    virtual bool TrsCmlToHeight(double dCml, double& dHeight, double& dFyj) = 0;
    
    ///计算与任意直线的交点
    virtual uint32_t IntersectWithLine(double dAngle, double dX, double dY, Point2d arrCrossPos[s_nMaxArrCount]) = 0;
...
protected:
    //相对里程->相对坐标
    virtual Point2d TrsCmlToNE_Relative(double dCml) = 0;
    
    //相对里程->相对角度
    virtual double TrsCmlToAngle_Relative(double dCml) = 0;
